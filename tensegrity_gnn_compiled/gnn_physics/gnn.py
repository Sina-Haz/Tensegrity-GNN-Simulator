...
class Encoder(nn.Module):
    ...
    def forward(self, graph: GraphData) -> GraphData:
        size = (graph.node_type.shape[0], self.n_out)
        graph['x'] = zeros(size, ref_tensor=graph.node_pos)
        ...
        for edge_name, n in graph.edge_type_map.items():
...
class RecurrentEncoder(Encoder):
    ...
    def __init__(self,
        ...
        self.node_encoders = nn.ParameterDict({
            name: mlp(in_dim, hidden_state_dim) for name, in_dim in node_types.items()
        })
...
class InteractionNetwork(nn.Module):
    ...
    def forward(self, graph: GraphData) -> GraphData:
        ...
        for name, mp in self.mp_dict.items():
            ...
            agg_x = mp(graph.x, edge_idx, edge_attr)

            # if name not in ['vn_gn', 'gn_vn']:
...
class RecurrentEncodeProcessDecode(EncodeProcessDecode):
    ...
    def __init__(
        ...
        self.node_and_hidden_mlp = mlp(2 * hidden_state_dim)
        self.hidden_node_proj_dict = {k: nn.Linear(latent_dim, hidden_state_dim)
                                      for k in node_types.keys()}

    def to(self, device):
        super().to(device)

        self.node_and_hidden_mlp = self.node_and_hidden_mlp.to(device)

        for k, v in self.hidden_node_proj_dict.items():
            self.hidden_node_proj_dict[k] = v.to(device)
    ...
    def forward(self,
        ...
        for k in self.hidden_node_proj_dict.keys():
            hidden_state = graph_batch[f'{k}_hidden_state'].to(graph_batch.x.device) \
                if f'{k}_hidden_state' in graph_batch \
                else torch.zeros_like(graph_batch.x)
            graph_batch['x'] = self.node_and_hidden_mlp(torch.hstack([
                graph_batch['x'],
                hidden_state
            ]))

        for k, proj in self.hidden_node_proj_dict.items():
            graph_batch[f'{k}_hidden_state'] = proj(graph_batch.x)
